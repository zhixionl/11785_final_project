...
def Extract_Angle(R):
    ...
    if not singular: 
        ...
        y = math.atan2(-R[2, 0], sy)
        z = 0
    #return x*180/np.pi, y*180/np.pi, z*180/np.pi  # if you want to use radio
    return x, y, z
...
def run_evaluation(model, dataset_name, dataset, result_file,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        # get information from two datasets 
        ...
        # # Get ground truth annotations from the batch if you have 
        ...
        if eval_pose:
            ...
            # exit()  # if you want to debug the test 
    ...
    if eval_pose:
        ...
        np.savez(checkpoint_filename,
                 MPJPE_0 = 1000 * mpjpe.mean(), 
                 reconstruction_0 = 1000 * recon_err.mean(),
                 MPJPE_avg = 1000 * mpjpe_avg.mean(), 
                 reconstruction_avg = 1000 * recon_err_avg.mean()
        )
